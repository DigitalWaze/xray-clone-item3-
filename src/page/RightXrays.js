...
class RightXrays extends Component {
    ...
    handleCheck = (checked, field, value) =>{
        let xray = this.state.xray;
        const activeTable = this.state.EvalType==="Hip" ? "RightHipImage" : "RightFrogImage";
    ...
    render() { 
                ...
                <tr>
                        <td  style={{verticalAlign:'bottom',}}>
                            <div style={{borderLeft:'4px solid #8fdf5d',borderRight:'4px solid #8fdf5d',width:'calc(100% - 0px)',height:'69px',lineHeight:'61px',marginBottom:'0px',verticalAlign:'bottom',display:'inline-block',borderTop:'4px solid #8fdf5d',borderBottom:'4px solid #8fdf5d'}}>
                                <span style={{display:'block',fontSize:'32px',fontWeight:'bold',color:'white'}}>Right Hip</span>
                            </div>
                        ...
                        <td colspan="3" style={{borderRight:global.state.FrogEval===true?'1px solid white':null,verticalAlign:'bottom'}}>
                            ...
                            {global.state.FrogEval===true?
                                <div style={{width:'50%',background:lB?'#8fdf5d':'',height:'69px',marginBottom:'0px',verticalAlign:'bottom',display:'inline-block',borderBottom:!lB?'1px solid white':null,borderTop:'1px solid white'}}>
                                    <a className="button-confirm w-button" onClick={global.toggleLateral} style={{ marginLeft:30,borderWidth:lB?3:0,borderColor:lB?'white':'none',borderStyle:lB?'solid':'none'}}>Lateral Hip</a>
                                </div>
                            :
                                <div style={{width:'50%',display:'inline-block',height:'57px',marginBottom:'0px',verticalAlign:'bottom',borderBottom:'1px solid white'}}>
                        ...
                        (<tr key={i.value} style={{borderLeft:'1px solid white'}}>
                            <td style={{verticalAlign:"bottom"}}>
                                ...
                                }
                                {
                                    ...
                                    <td  style={{borderRight:'1px solid white',borderBottom:'1px solid white'}}>
        ...
        <div style={{height:'calc(100vh - 35px)'}}>
            ...
            <a className="button-confirm w-button" onClick={this.prevImage} style={{position:"fixed", left:50, bottom:20}}>Previous Patient</a>
            <a className="button-confirm w-button" onClick={this.nextImage} style={{position:"fixed", left:"auto", right:50, bottom:20,fontWeight:'bold'}}> Evaluate Left Hip </a >

            <Note show={this.state.addNote} EvalType={this.state.EvalType==="Hip"?this.state.EvalType:'Right Lateral Hip'} notes={this.state.EvalType==="Hip"?[xray.value.rightNotes,xray.value.leftNotes]:this.state.EvalType==="Lateral"?xray.value.RightFrogImage.comment:''} onSubmit={(value)=>{
                if(this.state.EvalType==="Hip")
                {
