...
class LeftXrays extends Component {
  ...
  setComponent = async () => {
    ...
    if (xray && xray.value) {
      ...
      left = false;
      if (xray.value.LeftFrog) {
        FrogEval = true;
        this.setFrogImage(xray);
        // promises.push(
        //     getImageFromObject(xray.value.LeftFrogImage.imageUrl).then(base64 => {
        //         xray.value.LeftFrogImage.imageUrl.base64 = base64;
        //     })
        // );
      }
  ...
  handleCheck = (checked, field, value) => {
    let xray = this.state.xray;
    const activeTable =
      this.state.EvalType === "Hip" ? "LeftHipImage" : "LeftFrogImage";
  ...
  setFrogImage = async (xray) => {
    const FrogBase64 = await getImageFromObject(
      xray.value.LeftFrogImage.imageUrl
    );
          ...
          <Note
            ...
            notes={
              this.state.EvalType === "Hip"
                ...
                ? xray.value.LeftFrogImage.comment
                : ""
            }
            onSubmit={(value) => {
              if (this.state.EvalType === "Hip") {
